/**
 * This file contains tests for the RecursionException class.
 *
 * @copyright 2024 integereleven. All rights reserved. MIT license.
 */

import { assertEquals, describe, it } from '../dev_deps.ts';
import { RecursionExceptionData } from '../mod.ts';
...
describe('RecursionException', () => {
  it('inheritance tree', () => {
    const exception = new RecursionException('This is a test message.');
  ...
  it('new(message)', () => {
    fixturesNewMessage.forEach(([args, expected]) => {
      const [message] = args;
      const exception = new RecursionException(message);
  ...
  it('new(data)', () => {
    fixturesNewData.forEach(([args, expected]) => {
      const [data] = args;
      const exception = new RecursionException(data as RecursionExceptionData);
  ...
  it('new(message, data)', () => {
    fixturesNewMessageData.forEach(([args, expected]) => {
      ...
      const exception = new RecursionException(
        message,
        data as RecursionExceptionData,
      );
